...
public class SortPerf {
    ...
    public static void main(String[] args) {
        ...
        for (Sorter sorter : Sorter.values())
            System.out.print(" Speedup-" + sorter);
        System.out.println();

        for (ArrayBuilder ab : ArrayBuilder.values()) {
            for (int n : lengths) {
                java.util.Map<Sorter, Double> results = new java.util.HashMap<Sorter, Double>();
                ...
                for (Sorter sorter : Sorter.values()) {
					...
					results.put(sorter, minTime);
                }
                for(Sorter sorter : Sorter.values()) {
                    System.out.printf("%2f ",
                    		results.get(sorter) / results.get(Sorter.STIJNSORT));
                }
                System.out.println();
            }
        }
